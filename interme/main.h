...
// pressure force at previous step
const int force_pressure = 8;
...
template <typename CellList>
inline void calc_forces_Pressure(particles &vd, CellList &NN, double &max_visc)
{
    auto part = vd.getDomainIterator();
    ...
    while (part.isNext())
    {
       ...
       // No presssure force calculation for boundary particles
...
inline void calc_forces_withoutPressure(particles &vd, CellList &NN, double &max_visc)
{
    ...
    while (part.isNext())
    {
        ...
        vd.template getProp<force_pressure>(a)[0] = 0.0;
        vd.template getProp<force_pressure>(a)[1] = 0.0;
        vd.template getProp<force_pressure>(a)[2] = 0.0;
        vd.template getProp<drho>(a) = 0.0;
...
void max_acceleration_and_velocity(particles &vd, double &max_acc, double &max_vel)
{
    ...
    while (part.isNext())
    {
        ...
        Point<3, double> acc_f(vd.getProp<force>(a));
        Point<3, double> acc_p(vd.getProp<force_pressure>(a));
        Point<3, double> acc = acc_f + acc_p;
...
void verlet_int(particles &vd, double dt)
{
    ...
    while (part.isNext())
    {
        ...
        vd.template getProp<velocity>(a)[0] = vd.template getProp<velocity_prev>(a)[0] + (vd.template getProp<force>(a)[0] + vd.template getProp<force_pressure>(a)[0]) * dt2;
        vd.template getProp<velocity>(a)[1] = vd.template getProp<velocity_prev>(a)[1] + (vd.template getProp<force>(a)[1] + vd.template getProp<force_pressure>(a)[1]) * dt2;
        vd.template getProp<velocity>(a)[2] = vd.template getProp<velocity_prev>(a)[2] + (vd.template getProp<force>(a)[2] + vd.template getProp<force_pressure>(a)[2]) * dt2;
...
inline void EqState_incompressible(particles &vd, CellList &NN, double &dt, double &max_visc, double &rel_rho_predicted_error_max)
{
    ...
    while (it.isNext())
    {
        ...
        Point<3, double> va;
        // 	Predict velocity
        va[0] = vd.template getProp<velocity>(a)[0] + (vd.template getProp<force>(a)[0] + vd.template getProp<force_pressure>(a)[0]) * dt;
        va[1] = vd.template getProp<velocity>(a)[1] + (vd.template getProp<force>(a)[1] + vd.template getProp<force_pressure>(a)[1]) * dt;
        va[2] = vd.template getProp<velocity>(a)[2] + (vd.template getProp<force>(a)[2] + vd.template getProp<force_pressure>(a)[2]) * dt;

        //  Predict spatial Coordinates
        xa[0] = vd.getPos(a)[0] + vd.template getProp<velocity>(a)[0] * dt + (vd.template getProp<force>(a)[0] + vd.template getProp<force_pressure>(a)[0]) * dt205;
        xa[1] = vd.getPos(a)[1] + vd.template getProp<velocity>(a)[1] * dt + (vd.template getProp<force>(a)[1] + vd.template getProp<force_pressure>(a)[1]) * dt205;
        xa[2] = vd.getPos(a)[2] + vd.template getProp<velocity>(a)[2] * dt + (vd.template getProp<force>(a)[2] + vd.template getProp<force_pressure>(a)[2]) * dt205;

        /*
        drho update
        currently boundary and fluid particles are treated the same!
        */
        ...
        if (vd.getProp<type>(a) != FLUID)
        {
            ...
            while (Np.isNext() == true)
            {
                ...
                if (r2 < 4.0 * H * H)
                {
                    ...
                    double wab = Wab(r);
                    kernel += wab;
                    kernel_density += rhob * wab;
                    gravity_term += (dr.get(0) * wab * 0 + dr.get(1) * wab * 0 + dr.get(2) * wab * (-9.81));
                }
                ++Np;
            }
            vd.getProp<rho>(a) = kernel_density / kernel;
            vd.getProp<Pressure>(a) = (kernel_density + gravity_term) / kernel;
        }
        else
        {
            // Get an iterator over the neighborhood particles of p
            ...
            delta_pressure = pressureKoefficient * rho_predicted_error;

            // LOGDouble("Density Predicted error Koefficient = " , rho_predicted_error)  ;
            // LOGDouble("Pressure Koefficient = " , pressureKoefficient)  ;
            // LOGDouble("delta Presssure = "      ,   delta_pressure)     ;
